...
class UserProfilViewModel {
 ...
 static  alertDialog(BuildContext context)
  ...
  static void actionButtonDelete(BuildContext context, UsersModel model, Notifications notif, id) {
    showDialog(
        context: context,
        builder: (context) {
                               ...
                               child: ElevatedButton(
                                 onPressed:  (){
                                   if(GlobalFonction.connectionChecked == 1)
                                     {
                                       ServiceMethodes.delete(id); //delete on Api
                                       Widgets.toastMsg('usser delete succefully on API');
                                     }
                                   else
                                     {
                                       DatabaseManager.deleteBD(model.lastName); //delete on sqlite
                                       Widgets.toastMsg('usser delete succefully on local');
                                     }
